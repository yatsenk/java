...
//            System.out.println("   Array exchanges: " + s.getStep());
        s.qsort(m[11]); //я понял, как работает математика внутри
//            System.out.println("   Array exchanges: " + s.getStep());
        s.countingSort(m[12]); //кушает O(n) памяти со сложностью вычислений O(n),
                               // применим только на массивах с известным диапазоном значений
                               //также используется в Library Sort

//        s.bubbleSort3(m[0]); //с целью поупражняться
    }
}
